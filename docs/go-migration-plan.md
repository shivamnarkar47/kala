...
Port `kaal` (7,785 lines of Python across 17 `harness/` modules, 13,877 with tests) to a
single static Go binary. The port lands in this repo as `cmd/` + `internal/` packages,
Python stays untouched until a **parity gate** passes; then the Python tree is removed.
Core (dialect, gateway, messages, loop, context) stays **stdlib-only**; bubbletea family
(bubbletea, bubbles, lipgloss, glamour) serves the TUI; `golang.org/x/sync/errgroup` is
the only non-stdlib dependency in the core seam. Rough cost: **20 person-days across 7
phases** (~4 focused weeks, ~6 calendar weeks at half pace).

Why Go, in one breath: one static binary with no venv/interpreter bootstrap; goroutines +
`net/http` connection pooling replace the per-thread socket juggling and 4-worker thread
pools; `context` gives true cancellation (today's TUI Ctrl+C is cooperative, not forced);
and bubbletea's Elm-style `Cmd`/`Msg` model is purpose-built for exactly the
`AgentEvent`-stream seam `loop.py` already exposes.

The two hard things of kaal — DSML healing and `reasoning_content` replay — are the
**first** things ported, not the last, and are locked down with table-driven tests
ported verbatim from `tests/test_dialect.py` (299 lines) and `tests/test_messages.py`.

## 1. Motivation — what Go buys us
...
Non-goals (do not port): the Python tree is retired wholesale after the parity gate —
no long-term dual maintenance. The omp auth-store sqlite read (`~/.omp/agent/agent.db`)
is ported read-only with `modernc.org/sqlite` (pure Go, no cgo); if it proves fragile it
degrades to "env key / user key only" behind the same resolution order.
...
**Dependency budget** (hard rule, mirrors the stdlib-only core doctrine):

- `internal/dialect|gateway|messages|context|loop` → stdlib only (`net/http`, `encoding/json`,
  `bufio`, `os/exec`, `path/filepath`, `sync`, `crypto/sha256`) **plus** `golang.org/x/sync/errgroup`.
- `internal/tui` → bubbletea, bubbles, lipgloss, glamour (markdown rendering; the Python
  `Markdown` widget's counterpart). Nothing else may import these.
...
## 4. Concurrency model — the design center

The Python runtime's seams become Go's natural idioms; the **AgentEvent contract is
preserved byte-for-byte** so the TUI and `kaal run` stay interchangeable consumers.
...
## 5. TUI strategy — bubbletea

`tui.py` at 2,978 lines is the largest port. The split-pane surface maps cleanly:
...
Markdown rendering is the one real risk: glamour's output differs from Python's
markdown renderer. Parity target is **semantic** (headings, code blocks, lists) not
pixel; visual diffs are accepted and tuned in Phase 5.

## 6. The two hard things — ported first, locked by tests
...
2. **`reasoning_content` replay (`messages`).** Go's zero-value pitfall is real: an empty
   string is indistinguishable from an absent field. The wire struct uses
   `ReasoningContent *string` — replay happens iff the pointer is set — and a golden
   test asserts the turn-2 request body is byte-identical to the Python fixture (this is
   the exact 400-on-turn-2 trap, caught at unit level).
...
- **P0 — skeleton (1 d).** `go.mod` (Go 1.24), `cmd/kaal` stub with `--version` printing
  `kaal 0.1.0`, CI matrix (linux/macos/windows static builds), `install.sh`/`install.ps1`
  rewritten to fetch a release artifact, `kaal update` = release fetch. Python tree
  untouched; both trees coexist.
- **P1 — wire layer (3 d).** The two hard things (§6) + `gateway` with `httptest`
  fixtures ported from `tests/test_gateway.py` (511 lines): retry backoff 1s/2s/4s, no
  retry after visible content, 4xx immediate raise, `StreamEvent` typing.
- **P2 — loop + tools (4 d).** `AgentLoop` goroutine + `chan AgentEvent`; tool registry
  with path confinement (`filepath.Rel` + prefix check), DENY list, 10k-char cap, bash
  timeout (30 s default / 300 s max, `exec.CommandContext`), rg-backed grep with pure-Go
  fallback, `read` offset 1-based. Port `tests/test_loop.py::test_two_turn_tool_call_flow`
  first — it is the whole loop contract in one test. Tool-loop detection, 5-failure
  abort, exit-code mapping (0/1/2).
- **P3 — persistence (2 d).** JSONL sessions (`%Y%m%d-%H%M%S` ids, `KAAL_SESSIONS_DIR`
  override), resume replay, `.agent-memory` (200-line cap, 4k-token digest, verbatim
  dedupe), `structure` tree cache (depth ≤6, ≤20k entries, ≤500 lines, sig comment,
  first 120 lines into the prompt), `toolcache` (sha256|sig key, 4 MB cap, atomic
  temp+rename, mutator-batch bypass).
- **P4 — CLI (2 d).** cobra mirror of every flag and exit code in `kaal run --help`;
  stdin `-` prompt; `--json` final line; `--batch` with worker pool; verify hooks
  (`.kaal/hooks.json`, 30 s timeout, `[verify]` user message); `--no-tool-cache`,
  `--no-verify`, `--allow-dangerous`, `--resume`, `--dir` cwd confinement.
- **P5 — TUI (4 d).** bubbletea surface per §5; slash commands; session switcher;
  markdown parity tuning; `--verbose` reasoning pane. The `AgentEvent` channel makes
  this a consumer swap — the loop is not touched.
- **P6 — concurrency wave (2 d).** Profiling-driven: transport tuning (`MaxConnsPerHost`,
  idle timeouts), async session persistence behind flush-on-turn-end, pure-Go grep
  parallelization across files, batch worker autoscaling.
- **P7 — parity gate (2 d).** Run Python and Go binaries on the same corpus
  (20+ prompts drawn from `tests/` fixtures) and diff: final answers, session JSONL
  shape, tool-call sequences, reasoning replay on turn 2+. Gate = dialect/messages
  tables 100% green, wire bodies byte-identical on turn 2+, no corpus regressions.
  Then delete `harness/`, `tests/`, and the textual dependency — kaal is Go.
